...
function cti_flex_jquery_ui_tab($variables){
	$element = $variables['element'];
	$ui_tab = '<div id="tabs"><ul>';
	foreach ($element['header'] as $key => $value){
		$ui_tab .= '<li><a href="#'.$value->tabid.'">'.$value->title.'</a></li>';
	}
	$ui_tab .= '</ul>';
	foreach ($element['div'] as $key => $value){
		$ui_tab .= '<div id="'.$value->tabid.'">';
		$ui_tab .= '<p>'.$value->tabcontent.'</p>';
		$ui_tab .= '</div>';
	}
	$ui_tab .= '</div>';
	return $ui_tab;
